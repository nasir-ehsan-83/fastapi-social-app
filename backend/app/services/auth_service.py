...
from app.core.security import verify # to verfiry is the password hashed
...
async def login(user_credential: OAuth2PasswordRequestForm, db: AsyncSession):
    ...
    if user is None:
        raise HTTPException(satuse_code = status.HTTP_404_NOT_FOUND, detail = "Invalid credential")
